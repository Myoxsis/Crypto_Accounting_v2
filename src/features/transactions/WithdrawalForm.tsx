...
import { postFiatWithdrawal } from '../../services/posting'
...
export default function WithdrawalForm() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    const r = Number(rate)
    if (!amt || amt <= 0)
      return setSnack({ open: true, msg: 'Amount must be > 0', severity: 'error' })
    if (currency === 'EUR' && (!r || r <= 0))
      return setSnack({ open: true, msg: 'Provide a valid EUR→USD rate', severity: 'error' })

    setLoading(true)
    try {
      const ts = combineDateTime(date, time)
      await postFiatWithdrawal({
        ts,
        currency,
      ...
      setSnack({ open: true, msg: 'Withdrawal posted', severity: 'success' })
      setAmount('')
      setNotes('')
    } catch (e: any) {
